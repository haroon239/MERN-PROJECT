...
try {
	await axios.post("http://localhost:6500/signup", formdata).then((res)=>{
			console.log(res);
			if(res.data.status==200){
				Signin('/signin')
			}
			if(res.data.code===11000 && res.data.keyPattern && res.data.keyValue){
...
<div className="h-screen md:flex">
	<div className="relative overflow-hidden md:flex w-1/2 bg-black  i justify-around items-center hidden">
		<div className='px-[13%]'>
			<h1 className="text-white font-bold text-4xl font-sans mb-5">Welcome AutoVibes</h1>
			<p className="text-white mt-1">Unlock a world of automotive opportunities! Sign up now to buy, sell, or rent cars hassle-free. Your personalized dashboard awaits - simplified transactions, easy connections. Join today for a seamless car experience!</p>
...
{alert? <div className="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
  ...
  <div>
    <span className="font-medium">alert! Already exist :</span> {duplicat}
  </div>
</div>:""}
...
{/* inputs */}
            ...
            <IoPerson />
                ...
                <IoMdFingerPrint />
					...
					<div className="flex items-center border-2 py-2 px-3 rounded-2xl mb-4">
                    <MdAlternateEmail />
						<input className="pl-2 outline-none border-none"  type="email" placeholder="Email Address" autoComplete='off' onChange={emailvalidation} />
